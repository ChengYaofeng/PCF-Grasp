...
def train(args, global_config, train_dataset, test_dataset):
    ...
    ##tmp FLOPs calculation###
    # summary(model, (1024, 3))
    
    # input_tensor = torch.randn((1,1024,3)).cuda()
    ...
    for epoch in range(start_epoch, max_epoch):
        ...
        for batch_idx, (points, cam_poses, labels_dict) in enumerate(tqdm(train_dataloader, total=len(train_dataloader), smoothing=0.9)):
            ...
            # print(end_points['binary_score_head'])
        ...
        save_path = os.path.join(str(args.output_path), 'train', '{2}_{0}_best_ori_{1}.pth'.format(time.strftime("%m-%d-%H"), epoch, args.exp_name))
