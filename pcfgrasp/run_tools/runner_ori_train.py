...
def ori_train(args, global_config, train_dataset, test_dataset):
    ...
    test_dataloader = DataLoader(test_dataset, batch_size=['OPTIMIZER']['batch_size'], shuffle=False, num_workers=0, pin_memory=True, drop_last=True)                           
    #gpu指定

    model = CGNet(global_config)#.to(device)
    loss_cal = CGLoss(global_config)#.to(device)
    ...
    for epoch in range(start_epoch, max_epoch):
        ...
        for batch_idx, (points, cam_poses, labels_dict) in enumerate(tqdm(train_dataloader, total=len(train_dataloader), smoothing=0.9)):
            ...
            # print(batch_idx, offset_loss, approach_loss)
        ...
        save_path = os.path.join(str(args.output_path), 'train', '{0}_best_{1}.pth'.format(time.strftime("%m-%d-%H"), epoch))
