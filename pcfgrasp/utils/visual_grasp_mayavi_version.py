...
def visualize_grasps(full_pc, coarse, pred_grasps_cam, scores, obj_pc=None, plot_opencv_cam=True, pc_colors=None, gripper_openings=None, gripper_width=0.08):
    ...
    """
    # print(scores)  #<class 'numpy.ndarray'>
    ...
    draw_pc_with_colors(full_pc, pc_colors, single_color=(1, 0, 0))
    # draw_pc_with_colors(full_pc, single_color=(0.8, 0.1, 0.1))

    # if obj_pc is not None:
    #     draw_pc_with_colors(obj_pc, single_color=(0, 0.5, 0.5))
    if coarse is not None:
        draw_pc_with_colors(coarse, single_color=(0.5, 0.1, 0.1))


    # mlab.points3d(es_pred_pc[:, 0], es_pred_pc[:, 1], es_pred_pc[:, 2], color=(1, 0, 0), scale_factor=0.0018, mode = '2dsquare')
    ...
    colors2 = {k:cm2(0.5*0.5)[:3] for k in pred_grasps_cam if np.any(pred_grasps_cam[k])}
    ...
    for i,k in enumerate(pred_grasps_cam):
        ...
        if np.any(pred_grasps_cam[k]):
            ...
            if len(pred_grasps_cam) > 1:
                ...
                best_grasp.append([pred_grasps_cam[k][np.argmax(scores[k])]][0])
                # print(f'---------------------best_score:{scores[np.argmax(scores[k])]}-------------------') 
            else:
                ...
                print('---------------------best-score:{:.4f}--------------------'.format(scores[k][np.argmax(scores[k])]))
                # draw_grasps(pred_grasps_cam[k], np.eye(4), colors=colors3, gripper_openings=gripper_openings_k)
                best_grasp.append([pred_grasps_cam[k][np.argmax(scores[k])]][0])
    mlab.show()
...
def draw_pc_with_colors(pc, pc_colors=None, single_color=(0,1,0), mode='2dsquare', scale_factor=0.0018):
    """
    ...
    if pc_colors is None:
        # print('woshuotingting')
        # mlab.points3d(pc[:, 0], pc[:, 1], pc[:, 2], color=(1, 0, 0), scale_factor=scale_factor, mode = mode)
        ...
        points_mlab = mlab.points3d(pc[:, 0], pc[:, 1], pc[:, 2], scalars, mode=mode, scale_factor=0.0018)
...
def draw_grasps(grasps, cam_pose, gripper_openings, color=(1,0,0), colors=None, show_gripper_mesh=False, tube_radius=0.0008):
    """
...
def plot_grasp(pred_grasps_cam, gripper_openings=None, gripper_width=0.08):

    pred_grasps_cam = pred_grasps_cam.detach().numpy()
    # print(pred_grasps_cam)
    pred_grasps_cam = pred_grasps_cam.astype('int64')
    cm = plt.get_cmap('rainbow')
    cm2 = plt.get_cmap('gist_rainbow')
    ...
    # print(scores)
    # print(pred_grasps_cam)
    scores = np.ones_like(pred_grasps_cam)

    colors2 = (0.3,0.9,0.3)
    for k in pred_grasps_cam:
        # print(k)
        if np.any(k):
            gripper_openings_k = np.ones(len(k))*gripper_width if gripper_openings is None else gripper_openings[k]
            if len(pred_grasps_cam) > 1:
                draw_grasps(k, np.eye(4), color=colors2, gripper_openings=gripper_openings_k)    
                draw_grasps([k[np.argmax(scores[k])]], np.eye(4), color=colors2[k], 
                            gripper_openings=[gripper_openings_k[np.argmax(scores[k])]], tube_radius=0.0025)    
            else:
                colors3 = (0.3,0.9,0.3)
                draw_grasps(k, np.eye(4), colors=colors3, gripper_openings=gripper_openings_k)
