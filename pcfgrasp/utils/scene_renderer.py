...
from utils.mesh_utils import Object
...
class SceneRenderer:
    ...
    def change_scene(self, obj_paths, obj_scales, obj_transforms):
        ...
        #     scene_obj_pc.append(np.array(object_context['obj_pc'].vertices))
        
        # pc_all = np.concatenate(np.array(scene_obj_pc),axis=0)
    ...
    #     obj_context = self._load_object(p,s)
    #     obj_context = deepcopy(obj_context)

    #     self._scene.add_node(obj_context['node'])
    ...
    def _to_pointcloud(self, depth):
        ...
        if self._intrinsics in ['kinect_azure', 'realsense']:
            ...
            #np.where输出满足条件的坐标值，这里就是输出mask的坐标值
        ...
        world_z = depth[y, x]
        #这里是相机坐标系下的位置,因为没有涉及到相机的姿态,仅仅是从深度图像转换成pc
