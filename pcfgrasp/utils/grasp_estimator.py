import torch
import numpy as np
# from model.pcn_cgnet import PCGNet
from model.cgnet import CGNet
from model.pcfnet import PCFNet
# from model.pcfnet_use_coarse import PCFNet
from utils.data import preprocess_pc_for_task_inference, preprocess_pc_for_inference, farthest_points, \
    distance_by_translation_point, regularize_pc_point_count, reject_median_outliers, depth2pc
...
def select_grasps(contact_pts, contact_conf, max_farthest_points = 150, num_grasps = 50, first_thres = 0.23, second_thres = 0.19, with_replacement=False):
    ...
    """
    # print("执行select_grasp")
    # print(contact_conf.shape)

    grasp_conf = contact_conf.squeeze()
    # print(grasp_conf.shape)
    #把1的维度删掉
    ...
    # print(argsort(grasp_conf))
    # print(grasp_conf)
    # num_big = argsort(grasp_conf)
    # print(grasp_conf[num_big[200]])

    result = grasp_conf > first_thres
    # print(result)

    conf_idcs_greater_than = np.nonzero(result)[0]
    # print(conf_idcs_greater_than)
    ...
    if with_replacement:
        ...
        # print(len(conf_idcs_greater_than[center_indexes]))
        # print(num_grasps-len(conf_idcs_greater_than[center_indexes]))
        remaining_idcs = sorted_remaining_confidence_idcs[:num_grasps-len(conf_idcs_greater_than[center_indexes])]
        # print(len(remaining_idcs))
...
def extract_point_clouds(depth, K, segmap=None, rgb=None, z_range=[0.2,1.2], segmap_id=0, skip_border_objects=False, margin_px=5):
    ...
    ###############预处理
    num_points = 180  # 邻域球内的最少点数，低于该值的点为噪声点
    radius = 0.02
...
def extract_point_clouds_mask(depth, K, segmap=None, rgb=None, z_range=[0.5,0.8], segmap_id=0, skip_border_objects=False, margin_px=5):
    """
    Converts depth map + intrinsics to point cloud. 
    If segmap is given, also returns segmented point clouds. If rgb is given, also returns pc_colors.
    ...
    num_points = 180  # 邻域球内的最少点数，低于该值的点为噪声点     300
    radius = 0.01
    # sor_pcd, ind = pcd.remove_radius_outlier(num_points, radius)
    pcd = o3d.geometry.PointCloud()
    pcd.points = o3d.utility.Vector3dVector(pc_full)
...
#----------------------------------------------------------------------------------------------------------------------
    ...
    #     search_param=o3d.geometry.KDTreeSearchParamHybrid(radius=0.01, max_nn=180)
    # )
#----------------------------------------------------------------------------------------------------------------------
...
#----------------------------------------------------------------------------------------------------------------------
    ...
    # o3d.visualization.draw_geometries([sor_pcd])
#----------------------------------------------------------------------------------------------------------------------
    # o3d.io.write_point_cloud("/home/zjs/opencv_contrib/modules/surface_matching/samples/data/scene/mask_handle_extend_display.pcd", sor_pcd)
#----------------------------------------------------------------------------------------------------------------------

    # print(sor_pcd.normals)
    pc_full = np.array(sor_pcd.points)
    pc_colors = pc_colors[ind]
    # print(pc_colors)
    # print(pc_full.shape)
...
class GraspEstimatior():
    def __init__(self, cfg):
        ...
        #     # for sim evaluation
        #     self.global_config = config_utils.load_config(cfg['surface_grasp_logdir_folder'], batch_size=1, arg_configs=cfg.arg_configs)
    ...
    def predict_grasps(self, args, pc_full, obj_pc, constant_offset=False, convert_cam_coords=True, forward_passes=1):
        """
        ...
        # print(args.ori_inference)
        # args.ori_inference = False
        if args.ori_inference is True:
            print('-------------ori-inference------------')
            model = CGNet(self.global_config)
        else:
            print('-------------pcf-inference------------')
            model = PCFNet(args, self.global_config)
        ...
        with torch.no_grad():
            ...
            pc = torch.from_numpy(pc_batch).to(device)
            # print(pc.shape)
            start = time()
            end_points = model(pc)
            ...
            grasp_dir_head = grasp_dir_head.detach().cpu()#.numpy()
            grasp_offset_head = grasp_offset_head.detach().cpu()#.numpy()
            approach_dir_head = approach_dir_head.detach().cpu()#.numpy()
            ...
            if self.global_config['MODEL']['bin_offsets']:
                ...
                # thickness_gt = convert_tf_gather_nd(get_bin_vals(global_config), torch.unsqueeze(torch.argmax(offset_labels_pc, dim=2), dim=2))

            else:
                offset_bin_pred_vals = end_points['grasp_offset_pred'][:, :, 0]
                # thickness_gt = offset_labels_pc[:, :, 0]

            pred_grasps_cam = build_6d_grasp(approach_dir_head, grasp_dir_head, pred_points, offset_bin_pred_vals, use_cpu=True)

            # args.filter = False
            if args.filter is not True:
                print('----------------no filter----------------')
                ...
                if args.ori_inference is not True:
                    coarse_points = end_points['coarse'].cpu().squeeze().numpy()
                ...
                return pred_grasps_cam[selection_idcs], pred_scores[selection_idcs], pred_points[selection_idcs].squeeze(), gripper_openings[selection_idcs].squeeze(), coarse_points#, contact_pc, es_pc
            
            else:
                print('------------------------Filter----------------------')
                
                R_object_quaternion = np.asarray(args.rot_matrix) #wxyz
                R_object = R.from_quat(R_object_quaternion)
                R_object = R_object.as_matrix()
                R_rc = torch.tensor(R_object, dtype=torch.float32)
                T_rc = torch.tensor(args.tra_matrix, dtype=torch.float32)
                ...
                # print(A_gr.shape)  torch.Size([1, 1024, 1, 3])
                ...
                # a_gr = torch.matmul(pred_grasps_cam[:, :, :3, :3], R_rc.repeat(pred_grasps_cam.shape[0], pred_grasps_cam.shape[1], 1, 1))[:, :, :3, 2] #B N 3
                ...
                s = F.cosine_similarity(A_gr,a_gr, dim=2)
                grasp_score_weight = torch.sigmoid(s) #B N

                #ww
                #grasp_score_weight = torch.mul(a_z, grasp_score_weight) #B N
                ...
                return pred_grasps_cam[selection_idcs], pred_scores[selection_idcs], pred_points[selection_idcs].squeeze(), gripper_openings[selection_idcs].squeeze(), coarse_points

    def predict_scene_grasps(self, pc_full, args, obj_pc=None, pc_segments={}, local_regions=False, filter_grasps=True, forward_passes=1):
        """
        ...
        pred_grasps_cam[-1], scores[-1], contact_pts[-1], gripper_openings[-1], coarse= self.predict_grasps(args, pc_full, obj_pc, convert_cam_coords=False, forward_passes=forward_passes)
        ...
        return pred_grasps_cam, scores, contact_pts, gripper_openings, coarse

    def predict_scene_task_grasps(self, pc_full, args, obj_pc=None, pc_segments={}, local_regions=False, filter_grasps=True, forward_passes=1):
        """
        Predict num_point grasps on a full point cloud or in local box regions around point cloud segments.
        ...
        pred_grasps_cam, scores, contact_pts, gripper_openings, coarse, \
            center_idcs, selection_idcs, pred_points, pred_scores, pred_grasps_cam_whole = \
            self.predict_task_grasps(args, pc_full, obj_pc, convert_cam_coords=False, forward_passes=forward_passes)
        print('Generated {} grasps'.format(len(pred_grasps_cam)))
        print('Generated {} grasps'.format(len(pred_grasps_cam_whole)))
        print(pred_points.shape)

        return pred_grasps_cam, scores, contact_pts, gripper_openings, coarse, \
            center_idcs, selection_idcs, pred_points, pred_scores, pred_grasps_cam_whole
    
    def predict_task_grasps(self, args, pc_full, obj_pc, constant_offset=False, convert_cam_coords=True, forward_passes=1):
        """
        Predict raw grasps on point cloud
        ...
        pc, pc_mean, center_idcs = preprocess_pc_for_task_inference(pc_full.squeeze(), 1024, return_mean=True, convert_to_internal_coords=False)
        # print('pcf_model', pc_ori) #N x 3 numpy
        if obj_pc is not None:    #none 忽略
            obj_pc += np.expand_dims(pc_mean, 0)
            
        if len(pc.shape) == 2:   #忽略
            pc_batch = pc[np.newaxis, :, :]

        if forward_passes > 1:   #忽略
            pc_batch = np.tile(pc_batch, (forward_passes, 1, 1))
        ...
        with torch.no_grad():
            ...
            start_time = time()
            end_points = model(pc)
            print('Inference time: {}------------------'.format(time()-start_time))
            ...
            grasp_offset_head = grasp_offset_head.detach().cpu()#.numpy()
            approach_dir_head = approach_dir_head.detach().cpu()#.numpy()
            # approach_dir_head = torch.tensor([ 0., 0., 1.], dtype=torch.float32).repeat(approach_dir_head.shape[0], approach_dir_head.shape[1], 1) #绿色 1  红色 0 蓝色 2
            # grasp_dir_head = torch.tensor([ 0., 1., 0.], dtype=torch.float32).repeat(approach_dir_head.shape[0], approach_dir_head.shape[1], 1) #绿色 1  红色 0 蓝色 2

            pred_scores = pred_scores.detach().cpu()#.numpy()
            pred_points = pred_points.detach().cpu()#.numpy()
            # print('after_pcf_model', pred_points)
            ...
            if args.filter is not True:
                ...
                return pred_grasps_cam[selection_idcs], pred_scores[selection_idcs], pred_points[selection_idcs].squeeze(), gripper_openings[selection_idcs].squeeze(), \
                        coarse_points, center_idcs, selection_idcs, pred_points, pred_scores, pred_grasps_cam
                ...
                if convert_cam_coords:
                    pred_grasps_cam[:,:2, :] *= -1
                    pred_points[:,:2] *= -1
                return pred_grasps_cam[selection_idcs], pred_scores[selection_idcs], pred_points[selection_idcs].squeeze(), gripper_openings[selection_idcs].squeeze(), coarse_points, center_idcs, selection_idcs, pred_points, pred_scores, pred_grasps_cam
